...
def parseLine(line):
    ...
    if "(" in line and isName(line.split("(")[0].strip()):
        ...
        if line[i:].strip() == "":
            ...
            if line.split("(")[0].strip() == "sqrt":
                return {"type":"sqrt", "body":[parseLine(line[firstIndex+1:i-1].strip())]}
            if line.split("(")[0].strip() == "get":
                body = line[firstIndex+1:i-1].strip()
                i = 0
                parenDepth = 0
                insideString = False
                prevChar = ""
                while i < len(body):
                    if body[i] == "\"" and prevChar != "\\":
                        ...
                        parenDepth = parenDepth - 1
                    elif body[i] == "," and parenDepth == 0 and not insideString:
                        break
                    i = i + 1
                if i == len(body):
                    raise ValueError("The method 'get' requires two arguments.")
                arr = body[:i].strip()
                index = body[i+1:].strip()
                return {"type":"get", "arguments":[[parseLine(arr)],[parseLine(index)]]}
            if line.split("(")[0].strip() == "split":
                ...
                while i < len(body):
                    ...
                    i = i + 1
                if i == len(body):
                    raise ValueError("The method 'split' requires three arguments.")
                arr = body[:commaIndex].strip()
                index = body[commaIndex+1:i].strip()
                index2 = body[i+1:].strip()
                return {"type":"slice", "arguments":[[parseLine(arr)],[parseLine(index)],[parseLine(index2)]]}
            ...
            args = []
            while i < len(commas):
                args.append(body[prevComma+1:commas[i]])
                prevComma = commas[i]
                i = i + 1
            args.append(body[prevComma+1:-1])
            return {"type":"fnCall", "body":list(parseLine(arg) for arg in args), "name":line.split("(")[0].strip()}
    ...
    if line.strip().startswith("def "):
        lineData = line[4:].strip()
        i = 1
        while isName(lineData[:i]):
            i = i + 1
        functionName = lineData[:i-1].strip()
        rest = lineData[i-1:].strip()
        args = [[arg.strip().split(":")[0].strip(), arg.strip().split(":")[1].strip()]  for arg in rest[1:rest.index("]")].split(",")]
        body = parseLine(rest[rest.index("(")+1:-1])
        result = {"type":"function_declaration", "arguments":args, "body":[body], "name":functionName}
        return result
